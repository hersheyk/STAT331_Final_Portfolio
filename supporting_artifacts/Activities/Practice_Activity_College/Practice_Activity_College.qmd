...
```{r}
colleges_which<- colleges_clean  |> 
  filter(REGION == 7,STABBR != "ID", STABBR != "MT")|>
  mutate(above_med = (ADM_RATE> median(ADM_RATE)), 
         is_even = (SAT_AVG %% 2 != 0), UGDS = as.numeric(UGDS), 
         first_yr = (ADM_RATE * 1000) >= (0.25*UGDS ) 
         ) |>
  filter( 
         above_med == TRUE, 
         TUITION_DIFF != 0, 
         is_even == TRUE, first_yr == TRUE)
colleges_which
...
colleges_clean |> select("REGION", "STABBR", "INSTNM")
```
